...
class BugReportEmail(APIView):
    ...
    def post(self, request):
        SETTINGS = getattr(settings, "BUG_REPORTER", {
            "EMAIL_TEMPLATE_PATH": "__bug_report_email.html",
            "EMAIL_TO_ADDRESSES": ["prodissues@theonion.com"],
            "EMAIL_SUBJECT": "Hey! A bug was reported!"
        })
        ...
        body = loader.render_to_string(SETTINGS["EMAIL_TEMPLATE_PATH"], ctx)
        mail = EmailMessage(
            subject=subject,
            body=body,
            from_email=request.user.email
        )
        mail.to = SETTINGS["EMAIL_TO_ADDRESSES"]
        mail.send()
